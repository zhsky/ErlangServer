...
set PROCESSES=4
set MAKE_OPTS="{'DEBUG_MODE',true}"
set MAKE_LOCAL_OPTS="{'DEBUG_MODE',true},{'LOCAL_MODE',true}"
...
:fun_start
	...
	set SASL_LOG=./logs/log_%strtime%.sasl

	start werl +P 102400 -smp auto -pa "./ebin" -name %NODE_NAME% -setcookie %COOKIE% -hidden -kernel net_ticktime 600	-sasl sasl_error_logger {file,\"%SASL_LOG%\"} {errlog_type,error} -s main start
	goto wait_input
...
:fun_update
	start werl -pa "./ebin" -name update_%NODE_NAME% -setcookie %COOKIE% -eval "catch rpc:call('%NODE_NAME%',u,u,[]),init:stop()."
	goto wait_input
